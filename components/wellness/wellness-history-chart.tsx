...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div style={{ background: "white", padding: "12px 16px", borderRadius: 12, boxShadow: "0 8px 24px rgba(0,0,0,0.12)", fontSize: "0.82rem" }}>
      ...
      {d.readinessScore != null && <div style={{ color: "#22c55e" }}>Readiness: {d.readinessScore}</div>}
...
export function WellnessHistoryChart({ data }: { data: WellnessHistoryPoint[] }) {
  ...
  if (data.length < 2) {
    return (
      <div className="wellness-history-chart">
        <h3>状态趋势</h3>
        <p style={{ color: "var(--text-muted)", padding: "2rem 0", textAlign: "center" }}>历史数据不足，至少需要 2 天</p>
      </div>
    );
  }
  ...
  return (
    <div className="wellness-history-chart">
      ...
      <div className="wellness-chart-legend">
        ...
        <span><i style={{ background: "#22c55e" }} />Readiness</span>
      </div>
    </div>
  );
}
